...
export function CraftingProfitTable(): JSX.Element {
  ...
  return (
    <Table
      data={tableData}
      columns={columns}
      defaultSort={{ id: 'geProfitFromOre', desc: true }}
    />
  );
}
...
function useTableData(): TableRow[] {
  ...
  const jewelleryNames = jewellery.map((item) => item.name);
  const materials: Record<GemName | GoldType, OsrsItemData | undefined> = {
